            ...
            while c_begin < self.sizes[2]:
                ...
                while b_begin < len(queries):
                    ...
                    ranks[b_begin : b_begin + batch_size] += torch.sum(
                        (
                            torch.mul(scores_tem >= targets_tem, scores_cs > targets_cs)
                        ).float(),
                        dim=1,
                    ).cpu()
        ...
        self.embeddings = nn.ModuleList(
            [
                ...
                for s in [
                    ...
                    sizes[1],
                    1,
                ]  # last embedding modules contains no_time embeddings
            ]
        )
        ...
        for rel_1 in x[:, 1]:
            ...
            if rel_1_str in self.rule1_p2:
                ...
                for rel_2 in self.rule1_p2[rel_1_str]:
                    ...
                    rel2_emb = self.embeddings[3](
                        torch.LongTensor([int(rel_2)]).cuda()
                    )[0]
        ...
        for rel_1 in x[:, 1]:
            ...
            if rel_1_str in self.rule1_p2:
                ...
                for rel_2 in self.rule1_p2[rel_1_str]:
                    ...
                    tt = (
                        rel2_split[0] * transt[0][0],
                        rel2_split[1] * transt[0][0],
                        rel2_split[0] * transt[1][0],
                        rel2_split[1] * transt[1][0],
                    )
                    rtt = tt[0] - tt[3], tt[1] + tt[2]
                    # print("rel1_split:\t", rel1_split[0])
                    rule += weight_r * (
                        torch.sum(torch.abs(rel1_split[0] - rtt[0]) ** 3)
                        + torch.sum(torch.abs(rel1_split[1] - rtt[1]) ** 3)
                    )
                    rule_num += 1
        ...
        for rel_1 in x[:, 1]:
            if rel_1 in self.rule2_p1:
                ...
                for body in self.rule2_p1[rel_1]:
                    ...
                    tt3 = (
                        rel3_split[0] * transt[0][0],
                        rel3_split[1] * transt[0][0],
                        rel3_split[0] * transt[1][0],
