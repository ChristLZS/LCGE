...
with open(
    "./src_data/rulelearning/" + args.dataset + "/rule1_p2.json", "r"
) as load_rule1_p2:
...
model = {
    "LCGE": LCGE(
        sizes, args.rank, rules, args.weight_static, no_time_emb=args.no_time_emb
    ),
}[args.model]
model = model.cuda()
...
for epoch in range(args.max_epochs):
    ...
    if dataset.has_intervals():
        ...
        optimizer = TKBCOptimizer(
            model, emb_reg, time_reg, rule_reg, opt, batch_size=args.batch_size
        )
        optimizer.epoch(examples)
